...
<div id="view-edit-contract" class="fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5 border-bottom border-light pb-4">
        <h3 class="fw-bold text-dark m-0">Editează Contract</h3>
    </div>
    <div class="row justify-content-center">
        <div class="col-lg-10">
            <div class="spor-card p-5">
                <?= form_open('contracts/update/' . $contract['id'], ['id' => 'editContractForm']) ?>
                <?= csrf_field() ?>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-8">
                        ...
                        <input type="text" name="name" class="form-control" value="<?= old('name', $contract['name'] ?? '') ?>" placeholder="Ex: Contract Servicii IT" required>
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" name="contract_number" class="form-control" value="<?= old('contract_number', $contract['contract_number'] ?? '') ?>" placeholder="Ex: #CN-2024-001">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="client_name" class="form-control" value="<?= old('client_name', $contract['client_name'] ?? '') ?>" placeholder="Ex: Compania ABC SRL">
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-6">
                        ...
                        <?php if ($isDirector): ?>
                            <select name="region_id" class="form-select" required readonly disabled>
                                <?php foreach ($regions as $regionId => $regionName): ?>
                                    <option value="<?= $regionId ?>" <?= old('region_id', $contract['region_id'] ?? '') == $regionId ? 'selected' : '' ?>>
                                        <?= esc($regionName) ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                            <input type="hidden" name="region_id" value="<?= $contract['region_id'] ?>">
                            <div class="form-text text-muted small">Nu poți schimba regiunea contractului.</div>
                        <?php else: ?>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="manager_id" class="form-select">
                            ...
                            <?php foreach ($managers as $managerId => $managerName): ?>
                                <option value="<?= $managerId ?>" <?= old('manager_id', $contract['manager_id'] ?? '') == $managerId ? 'selected' : '' ?>>
                                    <?= esc($managerName) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <div class="form-text text-muted small">Poți schimba sau elimina managerul oricând.</div>
                ...
                <div class="row g-3 mb-4">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" name="progress_percentage" class="form-control" value="<?= old('progress_percentage', $contract['progress_percentage'] ?? 0) ?>" min="0" max="100" placeholder="0">
                ...
                <div class="mb-5">
                    ...
                    <select name="status" class="form-select" required>
                        <option value="planning" <?= old('status', $contract['status'] ?? 'planning') === 'planning' ? 'selected' : '' ?>>Planificare</option>
                        <option value="active" <?= old('status', $contract['status'] ?? '') === 'active' ? 'selected' : '' ?>>Activ</option>
                        <option value="on_hold" <?= old('status', $contract['status'] ?? '') === 'on_hold' ? 'selected' : '' ?>>În așteptare</option>
                        <option value="completed" <?= old('status', $contract['status'] ?? '') === 'completed' ? 'selected' : '' ?>>Finalizat</option>
                    </select>
