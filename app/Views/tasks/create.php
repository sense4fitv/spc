...
<div id="view-create-task" class="fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5 border-bottom border-light pb-4">
        <h3 class="fw-bold text-dark m-0">Creează Sarcină Nouă</h3>
    </div>
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="spor-card p-5">
                <?= form_open('tasks/store', ['id' => 'createTaskForm']) ?>
                <?= csrf_field() ?>
                ...
                <h6 class="fw-bold text-dark mb-4">Informații Sarcină</h6>
                <div class="mb-4">
                    ...
                    <select name="subdivision_id" id="select-subdivision" class="form-select" required>
                        ...
                        <?php foreach ($subdivisions as $subdivisionId => $subdivisionName): ?>
                            <option value="<?= $subdivisionId ?>" <?= old('subdivision_id') == $subdivisionId ? 'selected' : '' ?>>
                                <?= esc($subdivisionName) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" class="form-control" value="<?= old('title') ?>" placeholder="Ex: Predare Planuri Structura" required>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="4" placeholder="Descriere detaliată a sarcinii..."><?= old('description') ?></textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="priority" class="form-select" required>
                            ...
                            <?php foreach (['low' => 'Scăzută', 'medium' => 'Medie', 'high' => 'Ridicată', 'critical' => 'Critică'] as $priorityCode => $priorityLabel): ?>
                                <option value="<?= $priorityCode ?>" <?= old('priority', 'medium') === $priorityCode ? 'selected' : '' ?>>
                                    <?= $priorityLabel ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="assignees[]" id="select-assignees" class="form-select" multiple>
                        <?php foreach ($users as $userId => $userName): ?>
                            <option value="<?= $userId ?>" <?= in_array($userId, old('assignees', [])) ? 'selected' : '' ?>>
                                <?= esc($userName) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="mb-5">
                    ...
                    <div class="form-text text-muted small">Poți selecta mai multe departamente asociate sarcinii.</div>
                </div>

                <div class="d-flex justify-content-end gap-3">
                    <a href="<?= site_url('tasks') ?>" class="btn btn-spor-secondary px-4">Anulează</a>
                    <button type="submit" class="btn btn-spor-primary px-4">Creează Sarcină</button>
                </div>
                <?= form_close() ?>
            </div>
        </div>
