...
<div id="view-edit-issue" class="fade-in">
    <a href="<?= site_url('issues/view/' . $issue['id']) ?>" class="btn btn-link text-secondary text-decoration-none p-0 mb-4 fw-medium" style="font-size: 0.85rem;">
        <i class="bi bi-arrow-left me-1"></i> Înapoi la problematică
    </a>
    <div class="d-flex justify-content-between align-items-center mb-5 border-bottom border-light pb-4">
        <h3 class="fw-bold text-dark m-0">Editează Problematică</h3>
    </div>
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="spor-card p-5">
                <?= form_open('issues/update/' . $issue['id'], ['id' => 'editIssueForm']) ?>
                <?= csrf_field() ?>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="title" class="form-control" value="<?= old('title', $issue['title']) ?>" placeholder="Ex: Problemă cu acces la documente" required>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="6" placeholder="Descrie problematică în detaliu..."><?= old('description', $issue['description']) ?></textarea>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="region_id" id="select-region" class="form-select">
                            ...
                            <?php foreach ($regions as $regionId => $regionName): ?>
                                <option value="<?= $regionId ?>" <?= old('region_id', $issue['region_id']) == $regionId ? 'selected' : '' ?>>
                                    <?= esc($regionName) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="department_id" id="select-department" class="form-select">
                            ...
                            <?php foreach ($departments as $dept): ?>
                                <option value="<?= $dept['id'] ?>" <?= old('department_id', $issue['department_id']) == $dept['id'] ? 'selected' : '' ?>>
                                    <?= esc($dept['name']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="status" id="select-status" class="form-select" required>
                        <?php foreach ($statusLabels as $statusCode => $statusLabel): ?>
                            <option value="<?= $statusCode ?>" <?= old('status', $issue['status']) === $statusCode ? 'selected' : '' ?>>
                                <?= $statusLabel ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="form-text text-muted small">
                        Doar Admin poate arhiva problematici.
                    </div>
                ...
                <div class="d-flex justify-content-end gap-3">
                    <a href="<?= site_url('issues/view/' . $issue['id']) ?>" class="btn btn-spor-secondary px-4">Anulează</a>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const statusSelect = new TomSelect('#select-status', {
        create: false,
        sortField: {
            field: "text",
            direction: "asc"
    ...
    // Disable archived status if user is not admin
    const roleLevel = <?= $roleLevel ?>;
    if (roleLevel < 100) {
        statusSelect.disable();
        statusSelect.setValue('<?= $issue['status'] ?>');
        const archivedOption = statusSelect.getOption('archived');
        if (archivedOption) {
            statusSelect.removeOption('archived');
        }
        statusSelect.enable();
    }
});
