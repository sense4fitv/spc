...
<div id="view-add-subdivision" class="fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5 border-bottom border-light pb-4">
        <h3 class="fw-bold text-dark m-0">Adaugă Subdiviziune Nouă</h3>
    </div>
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="spor-card p-5">
                <?= form_open('subdivisions/store', ['id' => 'createSubdivisionForm']) ?>
                <?= csrf_field() ?>
                ...
                <div class="mb-4">
                    <label class="form-label small fw-bold text-muted text-uppercase">Contract <span class="text-danger">*</span></label>
                    <?php if ($isManager && count($contracts) === 1): ?>
                        <select name="contract_id" class="form-select" required>
                            <?php foreach ($contracts as $contractId => $contractName): ?>
                                <option value="<?= $contractId ?>" <?= old('contract_id', $selectedContractId) == $contractId ? 'selected' : '' ?>>
                                    <?= esc($contractName) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <div class="form-text text-muted small">Contractul tău disponibil.</div>
                    <?php else: ?>
                        <select name="contract_id" class="form-select" required>
                    ...
                    <?php endif; ?>
                    <?php if ($validation && $validation->hasError('contract_id')): ?>
                        <div class="text-danger small mt-1"><?= $validation->getError('contract_id') ?></div>
                    <?php endif; ?>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="code" class="form-control" value="<?= old('code') ?>" placeholder="Ex: SUB-01" required>
                    ...
                    <div class="col-md-8">
                        ...
                        <input type="text" name="name" class="form-control" value="<?= old('name') ?>" placeholder="Ex: Lucrări de construcție" required>
                ...
                <div class="mb-5">
                    ...
                    <textarea name="details" class="form-control" rows="3" placeholder="Detalii opționale despre subdiviziune..."><?= old('details') ?></textarea>
                ...
                <div class="d-flex justify-content-end gap-3">
                    ...
                    <button type="submit" class="btn btn-spor-primary px-4">Creează Subdiviziune</button>
                </div>
                <?= form_close() ?>
            </div>
        </div>
