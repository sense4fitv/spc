...
<div id="view-add-region" class="fade-in">
    ...
    <div class="d-flex justify-content-between align-items-center mb-5 border-bottom border-light pb-4">
        <h3 class="fw-bold text-dark m-0">Adaugă Regiune Nouă</h3>
    </div>
    <div class="row justify-content-center">
        <div class="col-lg-8">
            <div class="spor-card p-5">
                <?= form_open('regions/store', ['id' => 'createRegionForm']) ?>
                <?= csrf_field() ?>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" class="form-control" value="<?= old('name') ?>" placeholder="Ex: Muntenia Sud" required>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="description" class="form-control" rows="3" placeholder="Descriere opțională a regiunii..."><?= old('description') ?></textarea>
                ...
                <div class="mb-5">
                    ...
                    <select name="manager_id" class="form-select">
                        ...
                        <?php foreach ($directors as $director): ?>
                            <option value="<?= $director['id'] ?>" <?= old('manager_id') == $director['id'] ? 'selected' : '' ?>>
                                <?= esc($director['first_name'] ?? '') ?> <?= esc($director['last_name'] ?? '') ?> (<?= esc($director['email'] ?? '') ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="form-text text-muted small">Poți asigna un director regional mai târziu.</div>
                ...
                <div class="d-flex justify-content-end gap-3">
                    ...
                    <button type="submit" class="btn btn-spor-primary px-4">Creează Regiune</button>
                </div>
                <?= form_close() ?>
            </div>
        </div>
